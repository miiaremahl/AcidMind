...
public class OVRSkeleton : MonoBehaviour
{
	...
	public List<BoneId> ParticleBones;

	public GameObject trailSpot;
	...
	virtual protected void InitializeBones(OVRPlugin.Skeleton skeleton)
	{
		...
		for (int i = 0; i < skeleton.NumBones; ++i)
		{
			...
			//custom code for adding trails to hands
			if (ParticleBones.Contains(id))
            {
				GameObject trail=Instantiate(trailSpot, boneGO.transform.position, Quaternion.identity);
				trail.transform.parent = boneGO.transform;
	...
	//set the trails to be seen
	public void SetTrailsActive()
    {
